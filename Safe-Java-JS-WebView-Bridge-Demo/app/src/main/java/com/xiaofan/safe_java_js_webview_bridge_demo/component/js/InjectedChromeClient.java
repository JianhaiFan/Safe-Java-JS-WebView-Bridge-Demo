...
package com.xiaofan.safe_java_js_webview_bridge_demo.component.js;
...
public class InjectedChromeClient extends WebChromeClient {
    ...
    /**
     * 实例化InjectedChromeClient对象
     * @param injectedName  注入名
     * @param injectedCls   注入类
     */
    public InjectedChromeClient (String injectedName, Class injectedCls,ProgressWebView pwv,Context ctx) {
        mJsCallJava = new JsCallJava(injectedName, injectedCls);
        this.pwv = pwv;
        this.mContext = ctx;
    }

    public InjectedChromeClient (JsCallJava jsCallJava,ProgressWebView pwv,Context ctx) {
    ...
    public void onProgressChanged (WebView view, int newProgress) {
        ...
        if(NetWorkUtil.isNetworkConnected(mContext)) {
            if (!pwv.isProgressBarShow()) {
                pwv.showProgressBar();
            }
            pwv.setProgressBarValue(newProgress);

            if (newProgress >= 100) {
                // 进度条填充完成1秒之后隐藏进度条
                pwv.postDelayed(new Runnable() {
                    @Override
                    public void run() {
                        pwv.hideProgressBar();
                    }
    ...
    public void onReceivedTitle(WebView view, String title) {
        ...
        if (!TextUtils.isEmpty(title)) {
            if (title.contains("找不到网页") || title.contains("404") || title.toLowerCase().contains("error")) {
                view.stopLoading();
